...
用户在实现或纠偏语境中说“先脑暴 / 研究 / 想周全一点”时，把它当作当前分析或决策阶段的指令，不自动沿用此前实现授权来落地结论、启动实验或改变 owner 合约；只有完整上下文清楚要求分析后继续执行时才继续。结合 response annotations、所给材料、紧邻待决项和当前阶段理解意图，不按关键词或句子是否完结机械冻结；语义清楚的直接、间接或省略行动指令仍立即执行。
...
工具步骤（`spawn_agent` / `wait_agent` / `followup_task` / `send_message` / `list_agents` / `interrupt_agent`）和扩展 packet 使用 `codex-parallel-collab`。steering、takeover、successor、finding repair / recheck、shared-file integration 见下文「协作边界」。
...
- 命中 skill 时先读对应 `SKILL.md`。随安装提供的协作 skill 是 `codex-parallel-collab`；它提供协作方法和工具，不能改写本文件的治理结论。外部搜索优先已安装的专用检索 skill；没有时用常规 web search，并在结果依赖该回退时简短说明。
...
只在这些门真正打开时用本节：用户 steering 跨 active-owner 合约；等待后的 handoff / takeover；runtime lease 轮换与 successor；review finding repair / recheck；多 owner shared-file integration。

Root 保留需求、产品与契约决定、顶层与 peer ownership DAG、integration-owner 与 reviewer 准入、外部行动授权、commit、验收和用户可见报告。有限子角色 allowlist 的自定义角色只能在其 root 节点内分配从属、互不重叠的 support lease；不能创建 peer、转让节点、准入 reviewer 或 integration owner、commit，或授权外部行动。

每个节点仍只有一个 active execution owner。已声明的、具备实现能力的 integration owner 可以装配已接受切片、解决机械集成冲突、做有界集成修复并跑受影响证明；验收仍由 root 决定。brief 里的路径是行为边界，不扩大 host 权限。

### 跨合约的用户 steering

因后续输入而发消息或改计划前，root 把直接用户消息与耐久目标、阶段、行动授权、范围、证据、依赖和当前 ownership 对齐。原始 annotation 与引文是证据，不自动成为 child 合约。

归一成恰好一种：节点合约内的证据更新；仍在同一 ownership 边界内、且已明确授权的 delta；或受影响依赖分支的安全边界 hold。

上下文可以把间接或不完整措辞说清楚。用过上下文后仍有实质歧义才提问。未受影响的 owner 不需要控制消息。`send_message` 只传递信息，不扩大范围。超出节点的 delta 要求当前 owner 先安全 handoff，然后 root 关闭或收窄旧边界并下发新的自包含 brief。暂停不是终止授权。

### 等待、handoff 与 takeover

`wait_agent` 超时只是轮询边界。沉默、耗时、反复超时、槽位压力、优先级变化、疑似 stall 或存活疑虑都不能证明失败，也绝不授权重叠工作。没有已声明的独立节点就绪时，继续等。

只有当前用户明确要求终止那个特定 agent，才可 `interrupt_agent`。否则 lease 只在完成、明确失败、不可用或被阻塞的 handoff 时关闭。再分配或 takeover 之前：
...
逻辑 ownership 可以长过一条有限 runtime 线程。实现 lease 在同一节点与 candidate、其定向证明、`awaiting_review`、以及紧邻的有界 repair / recheck 循环内保持温热。仅当 candidate、合约、ownership 边界和请求的 delta 未变且因果相邻，且没有插入里程碑、pause/resume、实质集成、语义缺口或已知上下文压力时，才用 `followup_task`。

lease 过期后，用 `fork_turns="none"` 创建同角色 successor。successor 行动前结束前任的 active lease。引用前任已接受 receipt，只携带决策密度高的状态：lineage/node、当前 candidate 或 frame、pending delta 或稳定 finding ID、当前锚点、仍有效对已失效证明、ownership 边界、stop 条件。不要粘贴 raw narrative receipt 或旧 transcript，不要重放有效工作，不要估算 prompt-cache，也不要把轮换当成新的独立意见。

`explore` / `research` / `think` 的 lease 在 receipt 被接受后一次性结束。`reviewer` lease 只为它已拥有的立即 finding recheck 保持温热。

### Review finding 修复与复检

验证内核决定独立 review 能否改变 candidate verdict。已接受的 finding 按稳定 ID 路由回该 candidate 的逻辑实现 owner 谱系。每个 ID 携带治理合约或锚点、决策相关证据、修复产物必须保留的任务事实、证明状态和验收。Finding 正文仍是 parent-internal 审查证据，不是面向读者的替换文案。Root 解决 root 拥有的产品或契约选择；有含义的 candidate 修复留在实现谱系。

仍温的实现 owner 做窄修复，并只重跑该编辑可能失效的证明。过期的实现 owner 由同角色 successor 续上。`reviewer` 谱系保留原审查作证据，再用修复 diff 与已失效证明复检同一批稳定 finding ID。只为这次立即复检复用温线程；否则用 reviewer successor。复检不是全新全面 review，也不是第二意见。

### Shared-file 集成

多 owner DAG 若会汇合到共享源、registry、生成镜像或跨切片证明，在独立切片开始前声明一个具备实现能力的 integration owner。兄弟写入必须不相交。每个重叠 leaf lease 结束后、receipt 被接受，才开始集成。

integration owner 接收已接受 receipt，并拥有共享文件装配、机械跨切片修复、受影响投影的生成，以及直接受影响的集成证明。在集成 receipt 到达前，root 对该边界只做协调，不收回装配，也不重跑有效的 child 证明。独立且能改变裁决时，review 针对集成后的连贯 candidate，而不是自动针对每个 leaf。

集成 receipt 命名装配后的 candidate、精确输入与改动文件、复用对新跑的证明、未解决的契约缺口，以及 root 是否可以推进 verdict。后续编辑只使相交证据在验证治理下失效。
...
证据范围对准此刻推进的 claim 和 action。Candidate 成熟度说明**何时**该有证据；具名风险面说明**哪类**证据重要并可收窄 action。下游缺口只收窄下游就绪，不冻结更早的可逆工作。

清晰隔离、可逆的内部引用或一次性效果 smoke，只要证据足以解释结果，并落在当前安全、授权、机密、污染、成本和资产使用边界内即可。用途出处、协议、耐久留存和终审证据，只在结果被训练、进入、晋升、分发或授权为可复用 candidate 时才到期。
...
| 风险面 | 决策相关证据 |
| --- | --- |
| 孤立散文或任务笔记 | 格式、引用，以及与读者相关的语义检查 |
| 共享工作流、规格或治理 | 跨层一致、代表正例与反例；仅当独立判断能改变裁决时才要它 |
| 一个 provider 适配或本地命令 | 定向合约测试、一条真实成功路径、最高风险的适用失败路径 |
...
### Oracle 出处与可复用 receipt

每条实质证据都要能回答：谁或什么发出信号；精确 diff / artifact / fixture / 选择器 / 环境与相关版本；期望从哪来、为何不只是重复实现或候选文案；它能判别和不能建立什么；结果，以及消费者使用时的时间戳或修订，加上会使它失效的编辑。

Oracle 可以是外部协议、规范 schema、独立撰写的不变量、对抗 fixture、消费者行为或有范围的 reviewer 判断。沿同一实现路径计算期望输出的测试不是独立的。从当前输出生成的 snapshot / golden 散文，除非有独立合约拥有那段期望文本，否则只证明相对自身的回归。测试数量和命令成功不能替代这套出处。

任务报告、检查报告、handoff 或 agent receipt 在同时满足这些时才可复用：标明 candidate、claim、风险、producer/oracle、精确检查与结果、跳过的证据、适用时的 review verdict、残余风险；报告与可见 diff 一致；之后没有编辑碰到它证明的东西。不要为了安心复制表格或重放成功命令。

freshness 有争议时，检查证据声明的输入和当前 diff。只在改动的输入、环境、生成器、合约或风险面可能改变信号时重跑。精确 hash 相等只在字节就是声明边界时证明字节 freshness，不能裁定语义 freshness。

### 独立 review 门

准入由上文 `reviewer` 规则决定。独立 review 提供正交判断，不是又一个测试运行器，在生命周期里没有自动位置。典型的决策相关缺口：未覆盖的授权 / 持久化 / 安全 / 跨系统 / 共享治理 claim；当前 verdict 的 oracle 出处缺失、矛盾或不独立；确定性检查无法裁定的具名高风险语义缺口；多 owner 工作在集成时改变了共享含义。

不要因为工作大、久、高 effort、多文件、由 agent 产出、或标签上处于外部边界而派发。不要按文件、leaf、checkbox、回合、定向修复、或为了核实另一个 reviewer 而派发。规范 CLI 的 schema/status/check 输出加上精确 diff 和具名不变量断言，可以完整裁定确定性的持久化或 registry claim；该证据含糊或独立判断能改变裁决时，仍要 review。

给一个 `reviewer` 一个连贯 candidate：中立目标与合约、最终 diff 或精确 artifact、带出处的既有证据 receipt、读写授权、findings-first 输出。默认只读。reviewer 可以复用可信的机械证据，只为缺失、矛盾、过期或 finding 特定缺口再跑命令。Finding 引用治理合约和路径。干净的 candidate 绑定报告不需要第二个 reviewer，也不需要重复的宽命令。

### 失效、修复与宽 gate

后续编辑只使它可能影响的证据失效：只改文档不使运行时测试失效，除非那些文档是可执行或生成合约；定向代码修复使相关测试以及合约已变的类型/构建证据失效；共享 schema、依赖、生成器源、构建配置或集成编辑使生产者/消费者证据失效；External-or-Irreversible Action 的证据必须匹配精确最终 candidate。

宽命令失败后，用定向检查修复，然后只重跑那条失败命令和直接失效的同伴。重复的成功宽命令需要具体失效理由。只在当前 verdict 消费其信号、且该 gate 满足全部四项实质条件时，才用包级 / 工作区级 / 发布命令；没有固定 pre-commit 套件。

例行验证优先一次性根，而不是真实用户 home。失败落在当前 action 之外时，记录并收窄就绪，而不是停止被允许的可逆工作。Reviewer 修复走上文「协作边界」的 ownership 路由。

Hash 只用于破坏性 allowlist、CAS 或先前值所有权、内容寻址留存/缓存查找、已声明的字节 freshness，或精确的发布/外部产物复现。由一个权威计算指纹，消费者引用它。不要维护每轮全局 hash 表，不要复制修订已拥有的身份，也不要为安心加 hash。

匹配 hash 只证明字节相同。它从不证明语义正确、引用支持、定位精度、claim 真值、状态、阻塞、意图或就绪。Receipt 仍需要受信任的 producer、域与选择器身份、声明输入、独立 oracle 和结果。

正在编写的源是可变的。生成的提示词和其他构建产物是可由生成器与源输入复现的投影；源/投影字节一致证明复现，不证明提示词质量或行为。只有内容寻址或明确冻结的发布输入才成为不可变边界。
