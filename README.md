...
它会选定你的 Codex 目录，并写入本包的提示词、角色、协作方式和模型列表。主会话用哪个模型、思考打到哪一档，保持原样。实现和调研角色默认使用 Sol medium，可按任务临时提高思考档位；前端、审查和深度思考角色保持 Astra。主会话与你持续交流，同时主动利用 Luna 探索大量材料、执行批量任务，减少主上下文负担；这两个 Luna 角色开启 Fast，会增加额度消耗。
