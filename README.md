...
给 OpenAI Codex 用的一套配置：系统提示、六个编码角色、并行协作方式和模型列表。装进你这台机器上的 Codex 目录之后，写代码、查仓库、做调研和审查会按这套角色来分工。
...
它会选定你的 Codex 目录，并写入本包的提示词、角色、协作方式和模型列表。主会话用哪个模型、思考打到哪一档，保持原样。实现、调研、审查和深度思考角色使用 Astra。主会话与你持续交流，同时主动利用 Luna 探索大量材料、执行批量任务，减少主上下文负担；这两个 Luna 角色开启 Fast，会增加额度消耗。
